        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {POSTS.map((p, i) => (
            <motion.div
              ...
              whileHover={{ y: -5 }}
              className={`relative p-6 rounded-2xl border overflow-hidden cursor-default transition-all duration-300 ${dark ? "bg-white/[0.025] border-white/[0.07] hover:border-violet-500/25" : "bg-white border-black/[0.07] hover:border-violet-200"}`}
            >
              <div className={`absolute inset-0 bg-gradient-to-br ${p.grad} opacity-50`} />
              <div className="relative">
                <span className={`text-xs font-mono2 px-2.5 py-1 rounded-lg mb-4 inline-block ${dark ? "bg-white/10 text-white/55" : "bg-black/8 text-black/48"}`}>{p.cat}</span>
                <h3 className={`font-display font-bold text-sm leading-snug mb-4 ${dark ? "text-white" : "text-[#08080A]"}`}>{p.title[lang]}</h3>
                <div className={`flex items-center gap-2 text-xs font-body ${dark ? "text-white/30" : "text-black/30"}`}>
